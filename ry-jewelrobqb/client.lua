    ...
    while true do
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), -596.32, -283.82, 50.3236, true ) < 2 then
            ...
            if IsControlJustPressed(0, 38) then
                if not busy then
                    QBCore.Functions.TriggerCallback('ry-jewel:serversidecooldown2', function(atat)
                        if atat then
                            QBCore.Functions.TriggerCallback('jewel:CheckItem', function(success)
                                if success then 
                                busy = true
                    ...
                    QBCore.Functions.TriggerCallback('ry-jewel:serversidecooldown2', function(atat)
                        if not atat then
                            ...
                            if not ebasabilir then
                                QBCore.Functions.TriggerCallback('ry-jewel:serversidecooldown', function(atat)
                                    if atat then
                                        ebasabilir = true
                                        animasyon(kasaNo)
...
Citizen.CreateThread(function()
    while true do
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 902.143, 3564.90, 33.7968, true ) < 4 then
            ...
            if IsControlJustPressed(0, 38) then
                QBCore.Functions.TriggerCallback('CheckItem', function(count)
                    if count >= 1 then
